...
%% compute temperature correction factors
  ...
  aT_h = a_h/ TC_ah;      % d, age at hatch at f and 8.5C

  % birth
  L_b = L_m * l_b;                  % cm, structural length at birth of foetus  at f = 1
  Lw_b = L_b/ del_M;                % cm, total length at birth
  aT_b = t_b/ k_M/ TC_ab;   % d, age at birth at f and 8.5C
...
% tWde and tWde_E from NinnStev2006

vT = v * TC_tWde; kT_J = TC_tWde * k_J; JT_E_M = J_E_M * TC_tWde; JT_E_Am = J_E_Am * TC_tWde; 
UT_E0 = U_E0/ TC_tWde;
...
EWde_E = max(0, LUH(:,2) * JT_E_Am  - e_b * m_Em * LUH(:,1) .^ 3 * M_V) * w_E * 1e3; % predicted yolk in mg
 EWde_E =  EWde_E(2:end);
% embryo body mass
EWde = LUH(:,1).^3 * d_V * 1e3 * (1 + e_b * w); % predicted dry structure in mg
 EWde = EWde(2:end); 
