function dLEHRU = ode_LEHRU_bj(~, LEHRU,p,p_tox,f, c_d, param, male)
...
MQ  =  LEHRU(6); % nmolU/cm^3 structure if str = '' or nmol if str = 'absolute'
...
s   = (MQ - NEC)* (MQ>NEC) / M_QT; % parameter modification is linear in the internal concentration
...
E_G = E_G * 1;       
    ...
    case 'E_G'
E_G = E_G * (1 + s);   % J/cm^3, [E_G] cost for synthesis of a unit structure
...
dMQ   = k_e/l * P_Vd * c_d - MQ * (k_e/ l/ P_WV  + r );   % nmol/ cm^3, [M_Q], concentration of uranium per unit structure
